...
class CommentDelete(BaseHandler):
    @validate_csrf
    def post(self, comment_id): #topic_id
        comment = Comment.get_by_id(int(comment_id))
        #topic = Topic.get_by_id(int(topic_id))

        user = users.get_current_user()

        if comment.author_email == user.email() or users.is_current_user_admin(): # or topic.author_email == user.email()
...
class UserCommentsList(BaseHandler):#
    def get(self, user_id):
        ...
        comments = Comment.query(Comment.deleted == False).order(-Comment.created).fetch()
